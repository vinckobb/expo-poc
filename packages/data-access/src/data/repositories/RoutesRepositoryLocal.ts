...
export class RoutesRepositoryLocal implements RoutesRepository {
  ...
  async getRoutes(filter?: RouteFilter): Promise<Route[]> {
    const routes =
      (await this.localStorage.getItem<Route[]>(
        RoutesRepositoryLocal.ROUTES_KEY
      )) || [];
  ...
  async getFavoriteRoutes(filter?: RouteFilter): Promise<FavoriteRoute[]> {
    const favorites =
      (await this.localStorage.getItem<FavoriteRoute[]>(
        RoutesRepositoryLocal.FAVORITES_KEY
      )) || [];
  ...
  async getRouteDetails(routeId: string): Promise<Route> {
    const details =
      (await this.localStorage.getItem<Record<string, Route>>(
        RoutesRepositoryLocal.ROUTE_DETAILS_KEY
      )) || {};
  ...
  async addToFavorites(routeId: string): Promise<FavoriteRoute> {
    ...
    await this.localStorage.setItem(RoutesRepositoryLocal.FAVORITES_KEY, [
      ...favorites,
      newFavorite,
    ]);
    return newFavorite;
  ...
  async updateFavoritePosition(
    ...
    const updated = favorites.map((fav) =>
      fav.id === favoriteId ? { ...fav, position } : fav
    );
    await this.localStorage.setItem(
      RoutesRepositoryLocal.FAVORITES_KEY,
      updated
  ...
  private applyFilter(routes: Route[], filter?: RouteFilter): Route[] {
    ...
    if (filter.searchText) {
      ...
      result = result.filter((route) => {
        const matchName =
          filter.filterByName && route.name.toLowerCase().includes(search);
        const matchNumber =
          filter.filterByNumber && route.number.toLowerCase().includes(search);
